...
public class TuesdaySchedule extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					TuesdaySchedule frame = new TuesdaySchedule();
	...
	public TuesdaySchedule(ScheduleController scheduleController, ClassController classController) {
		...
		JLabel lblTodaysSchedule = new JLabel("Tuesday's Schedule:");
		...
		buttonRemove.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					scheduleController.removeEvent("Tuesday", list.getSelectedIndex());
					TuesdaySchedule tuesdaySchedule = new TuesdaySchedule(scheduleController, classController);
					tuesdaySchedule.setVisible(true);
					contentPane.setVisible(false);
					dispose();
			...
			public void mouseClicked(MouseEvent e) {
				...
				if ((eventTitle!=null) && (eventTitle.length()>0)) {
					String stringEventTime = (String)JOptionPane.showInputDialog(null, 
							...
							"Customized dialog", JOptionPane.PLAIN_MESSAGE, null, null, null);
					try {
						scheduleController.addEvent(eventTitle, "Tuesday", stringEventTime);
	...
	private void updateEventTitle(ScheduleController scheduleController, int index) {
		...
		if ((newEventTitle!=null) && (newEventTitle.length()>0)) {
			scheduleController.updateEventTitle("Tuesday", index, newEventTitle);
		}
	}
	...
	private void updateMeetingTime(ScheduleController scheduleController, int index) {
		...
		if ((newStringEventTime!=null) && (newStringEventTime.length()>0)) {
			try {
				scheduleController.updateMeetingTime("Tuesday", index, newStringEventTime);
			}
			catch (Exception exception){
				JOptionPane.showMessageDialog(null, "Invalid time of event entered. \n\nMake sure to "
						+ "enter as HH:MM-HH:MM, where HH refers to hour (ranging from 00 to "
