...
public class MondaySchedule extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					MondaySchedule frame = new MondaySchedule();
	...
	public MondaySchedule(ScheduleController scheduleController, ClassController classController) {
		...
		JList list = new JList(scheduleController.getEventsByDay("Monday"));
		...
		buttonRemove.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					scheduleController.removeEvent("Monday", list.getSelectedIndex());
					MondaySchedule mondaySchedule = new MondaySchedule(scheduleController, classController);
					mondaySchedule.setVisible(true);
					contentPane.setVisible(false);
					dispose();
			...
			public void mouseClicked(MouseEvent e) {
				...
				if ((eventTitle!=null) && (eventTitle.length()>0)) {
					String stringEventTime = (String)JOptionPane.showInputDialog(null, 
							...
							"Customized dialog", JOptionPane.PLAIN_MESSAGE, null, null, null);
					try {
						scheduleController.addEvent(eventTitle, "Monday", stringEventTime);
	...
	private void updateEventTitle(ScheduleController scheduleController, int index) {
		...
		if ((newEventTitle!=null) && (newEventTitle.length()>0)) {
			scheduleController.updateEventTitle("Monday", index, newEventTitle);
		}
	}
