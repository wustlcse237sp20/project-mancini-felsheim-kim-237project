...
public class WednesdaySchedule extends JFrame {
	...
	public WednesdaySchedule(ScheduleController scheduleController, ClassController classController) {
		...
		lblTodaysSchedule.setFont(new Font("Lucida Grande", Font.PLAIN, 20));
		...
		buttonRemove.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					scheduleController.removeEvent("Wednesday", list.getSelectedIndex());
					WednesdaySchedule wednesdaySchedule = new WednesdaySchedule(scheduleController, classController);
					wednesdaySchedule.setVisible(true);
					contentPane.setVisible(false);
					dispose();
			...
			public void mouseClicked(MouseEvent e) {
				...
				if ((eventTitle!=null) && (eventTitle.length()>0)) {
					String stringEventTime = (String)JOptionPane.showInputDialog(null, 
							...
							"Customized dialog", JOptionPane.PLAIN_MESSAGE, null, null, null);
					try {
						scheduleController.addEvent(eventTitle, "Wednesday", stringEventTime);
	...
	private void updateEventTitle(ScheduleController scheduleController, int index) {
		...
		if ((newEventTitle!=null) && (newEventTitle.length()>0)) {
			scheduleController.updateEventTitle("Wednesday", index, newEventTitle);
		}
	}
	...
	private void updateMeetingTime(ScheduleController scheduleController, int index) {
		...
		if ((newStringEventTime!=null) && (newStringEventTime.length()>0)) {
			try {
				scheduleController.updateMeetingTime("Wednesday", index, newStringEventTime);
			}
			catch (Exception exception){
				JOptionPane.showMessageDialog(null, "Invalid time of event entered. \n\nMake sure to "
						+ "enter as HH:MM-HH:MM, where HH refers to hour (ranging from 00 to "
