...
public class SundaySchedule extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					SundaySchedule frame = new SundaySchedule();
	...
	public SundaySchedule(ScheduleController scheduleController, ClassController classController) {
		...
		JLabel lblTodaysSchedule = new JLabel("Sunday's Schedule:");
		...
		JList list = new JList(scheduleController.getEventsByDay("Sunday"));
		...
		buttonRemove.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					scheduleController.removeEvent("Sunday", list.getSelectedIndex());
					SundaySchedule sundaySchedule = new SundaySchedule(scheduleController, classController);
					sundaySchedule.setVisible(true);
					contentPane.setVisible(false);
					dispose();
			...
			public void mouseClicked(MouseEvent e) {
				...
				if ((eventTitle!=null) && (eventTitle.length()>0)) {
					String stringEventTime = (String)JOptionPane.showInputDialog(null, 
							...
							"Customized dialog", JOptionPane.PLAIN_MESSAGE, null, null, null);
					try {
						scheduleController.addEvent(eventTitle, "Sunday", stringEventTime);
	...
	private void updateEventTitle(ScheduleController scheduleController, int index) {
		...
		if ((newEventTitle!=null) && (newEventTitle.length()>0)) {
			scheduleController.updateEventTitle("Sunday", index, newEventTitle);
		}
	}
	...
	private void updateMeetingTime(ScheduleController scheduleController, int index) {
		...
		if ((newStringEventTime!=null) && (newStringEventTime.length()>0)) {
			try {
				scheduleController.updateMeetingTime("Sunday", index, newStringEventTime);
