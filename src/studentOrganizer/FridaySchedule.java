...
public class FridaySchedule extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					FridaySchedule frame = new FridaySchedule();
	...
	public FridaySchedule(ScheduleController scheduleController, ClassController classController) {
		...
		JLabel lblTodaysSchedule = new JLabel("Friday's Schedule:");
		...
		JList list = new JList(scheduleController.getEventsByDay("Friday"));
		...
		buttonRemove.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					scheduleController.removeEvent("Friday", list.getSelectedIndex());
					FridaySchedule fridaySchedule = new FridaySchedule(scheduleController, classController);
					fridaySchedule.setVisible(true);
					contentPane.setVisible(false);
					dispose();
			...
			public void mouseClicked(MouseEvent e) {
				...
				if ((eventTitle!=null) && (eventTitle.length()>0)) {
					String stringEventTime = (String)JOptionPane.showInputDialog(null, 
							...
							"Customized dialog", JOptionPane.PLAIN_MESSAGE, null, null, null);
					try {
						scheduleController.addEvent(eventTitle, "Friday", stringEventTime);
	...
	private void updateEventTitle(ScheduleController scheduleController, int index) {
		...
		if ((newEventTitle!=null) && (newEventTitle.length()>0)) {
			scheduleController.updateEventTitle("Friday", index, newEventTitle);
		}
	}
	...
	private void updateMeetingTime(ScheduleController scheduleController, int index) {
		...
		if ((newStringEventTime!=null) && (newStringEventTime.length()>0)) {
			try {
				scheduleController.updateMeetingTime("Friday", index, newStringEventTime);
